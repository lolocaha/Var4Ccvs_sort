import csv
import os
filepath = input("Вставьте путь до файла: ")#Вставляем путь до файла
if filepath.find('\\') != -1:# Убираем лишние символы если они есть т.к при копировании пути на Windows 11 появляются лишние символы, пример:"C:\Users\User\PyCharmMiscProject\saf\ikm\po.csv"
...
def test():#Функция для проверки csv файла и вывода его
    try:
        with open(filepath,'r',encoding='utf-8-sig') as f: #открываем файл
    ...
    except FileNotFoundError:# Выводим ошибки если файл не найден или он неправильно заполнен
...
def report1(owners):#Меню для 1 задания
    ...
    with open('all_owners.csv',mode='w',newline='',encoding= 'utf-8-sig')as file:
        writer=csv.writer(file)
        for row in sorted_owners: #проходим по нему и выводим значения
            firsttask = []
            firsttask.append(f'ФИО:{row['Фамилия']} {row['Имя']} {row['Отчество']},Регистрационный номер: {row['Регистрационный номер']},Марка автомобиля: {row['Марка автомобиля']},Объем двигателя: {float(row['Объем двигателя'])},Год выпуска: {int(row['Год выпуска'])},Год постановки на учёт: {int(row['Год постановки на учёт'])}')
            writer.writerow(firsttask)#Записываем в файл данные и закрываем его
        file.close()
        if os.path.exists('all_owners.csv'):
            print(f'Файл all_owners создан по пути:{os.path.abspath("all_owners.csv")}')
def report2(owners): #Меню для 2 задания
    ...
    if len(vaz_owners)  == 0:#если список пустой, то выведет, что таких автомобилистов нет
        ...
        with open('VAZ_owners.csv', mode='w', newline='',encoding= 'utf-8-sig') as fi:
            writer=csv.writer(fi)
            for row in vaz_owners:
                secondtask = []
                secondtask.append(f'ФИО:{row['Фамилия']} {row['Имя']} {row['Отчество']},Регистрационный номер: {row['Регистрационный номер']},Марка автомобиля: {row['Марка автомобиля']},Объем двигателя: {float(row['Объем двигателя'])},Год выпуска: {int(row['Год выпуска'])},Год постановки на учёт: {int(row['Год постановки на учёт'])}')
                writer.writerow(secondtask)
            fi.close()
            if os.path.exists('VAZ_owners.csv'):
                print(f'Файл VAZ_owners.csv создан по пути:{os.path.abspath("VAZ_owners.csv")})')
...
def report3(owners):
    ...
    if len(old_cars) == 0:#если количество строк равно 0, то таких автомобилистов нет
        ...
        with open('Old_cars_owners.csv', mode='w', newline='',encoding='utf-8-sig') as fil:
            writer=csv.writer(fil)
            for i in old_cars:
                old_cars_sort= []
                old_cars_sort.append(f'ФИО:{i['Фамилия']} {i['Имя']} {i['Отчество']},Регистрационный номер: {i['Регистрационный номер']},Марка автомобиля: {i['Марка автомобиля']},Объем двигателя: {float(i['Объем двигателя'])},Год выпуска: {int(i['Год выпуска'])},Год постановки на учёт: {int(i['Год постановки на учёт'])}')
                writer.writerow(old_cars_sort)
            fil.close()
            if os.path.exists('Old_cars_owners.csv'):
                print(f'Файл Old_cars_owners.csv создан по пути:{os.path.abspath("old_cars_owners.csv")}')
